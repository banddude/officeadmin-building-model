...
def test_connectivity_golden_case_keeps_its_connectivity_through_ifc() -> None:
    """panel-to-evse declares both a port-to-port peer link and a route between
    the same two ports. Both must survive the export together."""

    model = load_golden_model(_case("panel-to-evse"))
    start, end = model.ports
    assert start.connected_port_ids == (end.id,)
    assert model.routes[0].start_port_id == start.id

    exported = round_trip(model)
    recovered = {port.id: port.connected_port_ids for port in exported.ports}

    assert recovered[start.id] == (end.id,)
    assert recovered[end.id] == (start.id,)


def test_no_port_exceeds_the_ifc4_connection_slots() -> None:
    """Forward invariant guard, not a regression reproduction.

    IFC4 bounds IfcPort.ConnectedTo and IfcPort.ConnectedFrom at SET [0:1]
    each. No current fixture breaches it, so this test does not fail on any
    version shipped so far; it exists to stop a future change from writing a
    second relationship into either role slot, which the schema forbids and
    ifcopenshell.validate does not check.
    """

    for name in sorted(EXPECTED_VALID_CASES):
        ifc = to_ifc(load_golden_model(_case(name)))
        for port in ifc.by_type("IfcPort"):
            assert len(port.ConnectedTo or ()) <= 1, name
            assert len(port.ConnectedFrom or ()) <= 1, name

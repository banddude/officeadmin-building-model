...
Ceiling-height annotations are spatially scoped. A note placed inside a resolved room enclosure (or explicitly naming one room) applies only to that room's `Space.height_m`, wall heights, and ceiling Z. A note that is explicitly global or is placed outside all resolved room enclosures can supply level-wide height evidence. Multiple distinct room heights on the same level therefore remain independent instead of being collapsed into one story height. `LevelOverride.height_m` and an explicitly supplied `ImportOptions.default_wall_height_m` remain level-wide inputs. If neither room-scoped nor level-wide height is supported, the importer may emit a 2D footprint-backed canonical `Space` but will not fabricate 3D walls or a ceiling.

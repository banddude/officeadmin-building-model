...
The first architectural plan that resolves level and scale and actually emits supported canonical spatial geometry may define the project-local XY origin. A resolved plan that emits no supported spatial geometry is recorded as `no_supported_geometry_recognized` with `architectural_geometry_unrecognized`; it does not claim the shared frame. Once a base geometry page exists, an explicit `RegistrationHint` remains authoritative when supplied. Before a later architectural sheet would otherwise use the sheet-geometry fallback, the importer tries to register it against an already-resolved drawing region of the same level with the shared wall matcher (#104): candidate-translation voting, endpoint verification, and the mirrored/rotated orientation guard, under the same thresholds (inliers, coverage, residual, spread, no competing translation, no mirrored or rotated alternative that explains the walls as well). Wall evidence is compared like with like — visible wall-layer segments on both sheets when both have them, otherwise paired wall faces. Several same-level regions may accept the sheet, but only when they place it identically in the canonical frame. An accepted sheet records frame basis `registered_to_region` with the target region, the matched evidence, and an `inferred` provenance entry; its confidence is the lower of the target frame's confidence and the wall method's 0.85. If the registration fails, the sheet-geometry fallback applies exactly as before: an additional CAD-vector page may use a lower-confidence sheet-geometry fallback — the importer prefers the largest proven closed wall-loop bounding box and otherwise uses non-title-block drawing extents, anchoring that geometry's lower-left at the project-local origin. Title-block vector geometry identified around explicit sheet/drawing/project metadata is excluded. The fallback method, confidence, source bounding box, and anchor are retained in page metadata and model provenance. Pages without enough vector geometry remain `registration_unresolved`.

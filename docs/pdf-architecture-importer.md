...
A level name is taken only from level/drawing-title text, never from a note that mentions a floor. A floor title may carry a short qualifier (`SECOND FLOOR PLAN - UNIT A`, `: AREA A`, ` (NORTH)`, at most three words); the text must start with the floor designation, so `SEE SECOND FLOOR FRAMING FOR BLOCKING` is not a level name. A drawing (or unsplit sheet) that names more than one distinct level is `level_ambiguous` and is not promoted, instead of taking the first name found.

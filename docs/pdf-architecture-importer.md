...
- room labels assigned by text position inside a closed wall loop: any nonempty text inside the enclosure is eligible unless it is a dimension, explicit keynote/note string, or recognized title-block string; numeric labels and project-specific abbreviations are supported without ROOM:/SPACE: prefixes, and label confidence/provenance is retained;

...
- Openings on the repeating sheet are hosted by the kept walls. A space the region emits names the kept wall ids. A geometric wall-loop space whose footprint matches an existing space of an evidence-linked frame on the level within 0.05 m is not emitted twice; one whose stable identity is already claimed otherwise is recorded as `duplicate_room_identity_across_pages` and not emitted.

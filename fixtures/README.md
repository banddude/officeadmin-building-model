...
8. `geometry-only-power-sheet-notes-column-legend.pdf`: Slice 9/12 source-only acceptance PDF with a right-hand notes column, separate bottom title block, KEY NOTES and GENERAL NOTES sections, an eight-row ruled `SYMBOL | FUNCTION` legend covering duplex/quad receptacles, data/combination outlets, power/data junction boxes, access control and CATV, plus two field instances per type at 1.0x and 0.8x (including axis mirroring), E/N status tags, bare +44\" height annotations and circuit-count numbers. There is no paired expected-output artifact.

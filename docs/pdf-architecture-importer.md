...
- Openings on the repeating sheet are hosted by the kept walls. A space the region emits names the kept wall ids. A geometric wall-loop space whose footprint matches an existing space on the level within 0.05 m is not emitted twice.
...
- Walls that only resemble an emitted wall stay distinct and are emitted: the same plan on another level, a parallel wall beside it, and a collinear run that is longer or shifted beyond the thickness.

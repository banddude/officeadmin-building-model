...
A wall is materialized once per canonical identity. When a region would emit a wall whose level/geometry identity an earlier drawing region already emitted (the same plan set often draws one level several times), the repeated walls are not emitted again: the region records `duplicate_wall_identity_across_pages` with the earlier regions, which also appear in its `repeated_geometry_region_ids`. A region whose every entity repeats earlier regions is `no_supported_geometry` with `repeated_geometry_not_reemitted` — its recognition succeeded, nothing was unrecognized — and it does not join the resolved regions, so electrical sheet registration does not see it as an additional frame target.
